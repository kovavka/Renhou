...
describe('hasTiles', () => {
    ...
    it('should find 2 tiles in a list if exist', () => {
        const tiles = getTilesFromString('234m')
        expect(hasTiles(tiles, {type: SuitType.MANZU, value: 2}, {type: SuitType.MANZU, value: 3})).toBe(true)
    })
    it('should find 2 different suit tiles in a list', () => {
        const tiles = getTilesFromString('23m9s')
        expect(hasTiles(tiles, {type: SuitType.MANZU, value: 2}, {type: SuitType.SOUZU, value: 9})).toBe(true)
    })
